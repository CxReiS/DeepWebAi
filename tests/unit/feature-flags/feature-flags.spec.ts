...
import { FeatureFlagManager, FeatureFlagConfig, UserContext } from '../../../packages/feature-flags';
...
describe('Feature Flags', () => {
  ...
  describe('FeatureFlagManager', () => {
    ...
    it('should throw error when GrowthBook key is missing for GrowthBook provider', () => {
      ...
      }).toThrow('GrowthBook client key is required when using GrowthBook provider');
    });
  });
  ...
  describe('Utility Methods', () => {
    ...
    it('should create user context correctly', async () => {
      const userContext = await featureFlagManager.createUserContext('user-123', {
  ...
  describe('Feature Flag Constants', () => {
    it('should have predefined feature flags', () => {
      const { FEATURE_FLAGS } = require('../../../packages/feature-flags');
      ...
      expect(FEATURE_FLAGS.REALTIME_COLLABORATION).toBe('realtime-collaboration');
    });
  });
});
