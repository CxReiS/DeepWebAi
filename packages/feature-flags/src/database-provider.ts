...
function createNoopPool(): Pool {
  return {
    async connect(): Promise<PoolClientLike> {
      return {
        // Return minimal shapes expected by tests
        async query() {
          return { rows: [] } as any;
        },
        release() {
...
export class DatabaseFeatureFlagProvider implements FeatureFlagProvider {
  ...
  constructor(config: FeatureFlagConfig) {
    this.config = config;
    // In production, instantiate a real Pool using `pg`. For tests, use a noop pool.
    if (process.env.NODE_ENV === "test") {
      this.pool = createNoopPool();
    } else {
      // Defer real pool creation to runtime environment to avoid hard dependency on `pg` here
      this.pool = createNoopPool();
    }
  }
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.error(
        "Failed to initialize database feature flag provider:",
        error
      );
      throw error;
    }
  ...
  async isFeatureEnabled(
    ...
    if (
      cached &&
      Date.now() - cached.timestamp < this.config.cacheTimeout * 1000
    ) {
      return cached.value;
    }
    ...
    try {
      ...
      await client.query(
        `UPDATE feature_flags SET ${setParts.join(
          ", "
        )}, updated_at = CURRENT_TIMESTAMP WHERE name = $${paramIndex}`,
        [...values, name]
      );
